...
def app(tskin: TSkin, keyboard: KeyboardController, braccio: Optional[BraccioInterface], actions: List[ShapesPostAction], debug_queue: Queue):
    global direction
    if check_touch(tskin, "SINGLE_TAP", actions):
        if direction:
            braccio_gripper(braccio, debug_queue, Gripper['OPEN'])
            braccio_move(braccio, debug_queue, 0, 100, 100)
            braccio_move(braccio, debug_queue, 0, 100, (-20))
            braccio_gripper(braccio, debug_queue, Gripper['CLOSE'])
            braccio_move(braccio, debug_queue, 0, 100, 100)
            braccio_move(braccio, debug_queue, 100, 0, 100)
            braccio_move(braccio, debug_queue, 100, 0, (-20))
            braccio_gripper(braccio, debug_queue, Gripper['OPEN'])
            braccio_move(braccio, debug_queue, 100, 0, 100)
            braccio_gripper(braccio, debug_queue, Gripper['CLOSE'])
        else:
